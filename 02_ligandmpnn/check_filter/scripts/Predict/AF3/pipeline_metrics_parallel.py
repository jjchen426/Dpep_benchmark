...
def compute_sc_rmsd_with_prepared_ref(
    ...
    if len(pred_atoms) == 0:
        return None
    if len(pred_atoms) != len(ref_coords):
        return None
...
def process_parent_group(
    ...
    if not ref_pdb_path.exists():
        skip_reasons["missing_ref_pdb"] = len(design_dir_paths)
        return {
            "rows": rows,
            "matched_design_dirs": matched_design_dirs,
    ...
    try:
        ...
        skip_reasons["missing_peptide_chain_in_ref"] = len(design_dir_paths)
        return {
            "rows": rows,
            "matched_design_dirs": matched_design_dirs,
            "total_sample_dirs": total_sample_dirs,
    ...
    if align_chain_ref is None:
        skip_reasons["missing_align_chain_in_ref"] = len(design_dir_paths)
        return {
            "rows": rows,
            "matched_design_dirs": matched_design_dirs,
            "total_sample_dirs": total_sample_dirs,
    ...
    if len(align_ref_atoms) == 0 or len(ref_atoms) == 0:
        skip_reasons["missing_backbone_atoms_in_ref"] = len(design_dir_paths)
        return {
            "rows": rows,
            "matched_design_dirs": matched_design_dirs,
            "total_sample_dirs": total_sample_dirs,
...
def merge_skip_reasons(target: dict[str, int], source: dict[str, int]) -> None:
...
def collect_metrics_for_output_dir(
    ...
    parent_to_design_dirs: dict[str, list[str]] = {}
    for design_dir in sorted(design_dirs):
        ...
        if parent_complex not in pdb_set:
            ...
            continue
        parent_to_design_dirs.setdefault(parent_complex, []).append(str(design_dir))
    ...
    completed_groups = 0
    if num_workers == 1:
        for parent_complex, design_dir_paths in parent_items:
            result = process_parent_group(
                parent_complex=parent_complex,
                design_dir_paths=design_dir_paths,
                ref_pdb_base=str(ref_pdb_base),
                peptide_chain_id=peptide_chain_id,
                backbone_atoms=backbone_atoms,
            )
            rows.extend(result["rows"])
            matched_design_dirs += result["matched_design_dirs"]
            total_sample_dirs += result["total_sample_dirs"]
            merge_skip_reasons(skip_reasons, result["skip_reasons"])
