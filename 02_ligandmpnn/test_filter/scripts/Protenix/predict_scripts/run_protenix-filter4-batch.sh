...
for pdb in $(cat /home/junjiechen/1_work/250401-Dpepalign/Benchmark-Filter/datasets/PepSet_3per_length/PDB.list); do
    ...
    echo "Running protenix predict in directory: $pdb-merged"
    CUDA_VISIBLE_DEVICES=1 protenix predict --input pred_filter4.json --out_dir ./predict_output-filter4 --use_msa true --seeds 42,43,44 > protenix_predict-filter4.log 2>&1
    echo "Finished protenix predict in directory: $pdb-merged"
    echo "----------------------------------------"
    cd ..
    cd ..
done
