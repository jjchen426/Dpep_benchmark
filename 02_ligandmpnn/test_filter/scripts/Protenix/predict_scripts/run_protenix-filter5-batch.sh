...
for pdb in $(cat /home/junjiechen/1_work/250401-Dpepalign/Benchmark-Filter/datasets/PepSet_3per_length/PDB.list); do
    cd 11000-0.1_0.2T
    cd $pdb
    echo "Running protenix predict in directory: $pdb with filter5"
    CUDA_VISIBLE_DEVICES=1 protenix predict --input pred_filter5.json --out_dir ./predict_output-filter5 --use_msa true --seeds 42,43,44 > protenix_predict-filter5.log 2>&1
