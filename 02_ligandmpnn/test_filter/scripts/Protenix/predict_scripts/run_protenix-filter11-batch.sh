...
for pdb in $(cat /home/junjiechen/1_work/250401-Dpepalign/Benchmark-Filter/datasets/PepSet_3per_length/PDB.list); do
    ...
    CUDA_VISIBLE_DEVICES=0 protenix predict --input pred_filter11.json --out_dir ./predict_output-filter11 --use_msa true --seeds 42,43,44 > protenix_predict-filter11.log 2>&1
    echo "Finished protenix predict in directory: $pdb -filter11"
    echo "----------------------------------------"
    cd ..
    cd ..
done
