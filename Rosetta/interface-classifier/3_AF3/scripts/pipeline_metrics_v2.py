...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--peptide-chain-ref", type=str, default="L",
                        help="ref PDB 中的多肽链 ID（默认: L）")
    parser.add_argument("--peptide-chain-pred", type=str, default="B",
                        help="AF3 预测中的多肽链 ID（默认: B）")
...
def validate_args(args: argparse.Namespace) -> tuple[set[str], list[float], int]:
    ...
    peptide_chain_ref = args.peptide_chain_ref.strip()
    if not peptide_chain_ref:
        raise ValueError("--peptide-chain-ref 不能为空")
    peptide_chain_pred = args.peptide_chain_pred.strip()
    if not peptide_chain_pred:
        raise ValueError("--peptide-chain-pred 不能为空")
    ...
    return backbone_atoms, cutoffs, resolved_workers, peptide_chain_ref, peptide_chain_pred
...
def extract_backbone_coords(cif_path: str, chain_ids: set[str] | None,
    ...
    with open(cif_path) as f:
        for line in f:
            ...
            if in_atom_site and col_names and not line.startswith("_"):
                ...
                res_dict = raw.setdefault(chain, {}).setdefault(res_id, {})
                res_dict[atom] = (x, y, z)

    # 按残基索引 + 原子顺序展平
    result: dict[str, np.ndarray] = {}
    chain_iter = chain_ids if chain_ids is not None else raw.keys()
    for cid in chain_iter:
        if cid not in raw:
            continue
        coords: list[float] = []
...
def select_non_peptide_chain_from_model(model, peptide_chain_id: str,
                                        backbone_atoms: set[str],
                                        ) -> tuple[str, np.ndarray] | None:
    """从 ref 的 Biopython model 中选择非多肽链。

    返回 (选中的链 ID, 坐标)。若没有候选链则返回 None。
    """
    candidates: list[tuple[str, np.ndarray]] = []
    for chain in model:
        if chain.id == peptide_chain_id:
            continue
        coords = get_sorted_backbone_coords(chain, backbone_atoms)
        if coords.size == 0:
            continue
        candidates.append((chain.id, coords))

    if not candidates:
        return None

    ordered = sorted(candidates, key=lambda x: x[1].shape[0], reverse=True)
    chosen_id, chosen_coords = ordered[0]
    return chosen_id, chosen_coords


def select_non_peptide_chain_from_coords(coords: dict[str, np.ndarray],
    ...
    返回 (选中的链 ID, 坐标)。若没有候选链则返回 None。
    """
    candidates = [(cid, arr) for cid, arr in coords.items() if cid != peptide_chain_id]
    if not candidates:
        return None
...
def process_ref_group(ref_name: str, design_names: list[str],
                      ...
                      peptide_chain_pred: str, peptide_chain_ref: str,
                      backbone_atoms: set[str],
                      ) -> tuple[list[tuple], int]:
    """处理共享同一个 ref 的所有设计目录。

    每个 ref PDB 只解析一次，之后对所有属于该 ref 的预测 sample 计算 RMSD。
    *peptide_chain_pred* — AF3 预测中的多肽链 ID
    *peptide_chain_ref*  — ref PDB 中的多肽链 ID
    返回 (rows, sample_count)。
    """
    pdbparser = PDBParser(QUIET=True)

    # ── 解析 ref PDB（按 peptide_chain_ref 提取多肽；非多肽链单独选择）──
    ref_path = Path(ref_pdb_base) / f"{ref_name}.pdb"
    ref_struct = pdbparser.get_structure("ref", str(ref_path))
    try:
        ref_pep_chain = ref_struct[0][peptide_chain_ref]
    except KeyError:
        print(f"[跳过] {ref_name}: ref 缺少多肽链 {peptide_chain_ref}", flush=True)
        return [], 0

    ref_pep_coords = get_sorted_backbone_coords(ref_pep_chain, backbone_atoms)
    if ref_pep_coords.size == 0:
        print(f"[跳过] {ref_name}: ref 多肽链 {peptide_chain_ref} 无骨架原子", flush=True)
        return [], 0

    ref_align = select_non_peptide_chain_from_model(
        ref_struct[0], peptide_chain_ref, backbone_atoms,
    )
    if ref_align is None:
        print(f"[跳过] {ref_name}: ref 缺少非多肽链", flush=True)
        return [], 0
    ref_align_chain_id, ref_align_coords = ref_align
        ...
        for sample_dir in sample_dirs:
            ...
            try:
                pep_plddt, plddt, iptm, ptm, pep_ptm, pae_min, ipae, ranking_score = \
                    extract_sample_metrics(conf_path, summary_path, peptide_chain_pred)
            except (KeyError, json.JSONDecodeError, OSError) as e:
                print(f"  [跳过] {sample_dir.name}: JSON 读取失败 ({e})", flush=True)
                continue

            # 轻量 CIF 解析：pred 链独立选择（不假设与 ref 链 ID 一致）
            coords = extract_backbone_coords(
                str(cif_path), None, backbone_atoms,
            )
            pred_pep_coords = coords.get(peptide_chain_pred)
            if pred_pep_coords is None:
                print(f"  [跳过] {prefix}: pred 缺少多肽链 {peptide_chain_pred}", flush=True)
                continue

            pred_align = select_non_peptide_chain_from_coords(
                coords, peptide_chain_pred,
            )
            if pred_align is None:
                print(f"  [跳过] {prefix}: pred 缺少非多肽链", flush=True)
                continue
            pred_align_chain_id, pred_align_coords = pred_align

            if len(pred_pep_coords) != len(ref_pep_coords):
                raise RuntimeError(
                    f"{prefix}: 多肽链长度不匹配 (pred={len(pred_pep_coords)}, ref={len(ref_pep_coords)})"
                )
            if len(pred_align_coords) != len(ref_align_coords):
                raise RuntimeError(
                    f"{prefix}: 对齐链长度不匹配 (pred={len(pred_align_coords)}, ref={len(ref_align_coords)})"
                )
    ...
    if num_workers == 1:
        for ref_name, design_names in group_items:
            ...
            rows, n_samples = process_ref_group(
                ...
                peptide_chain_pred, peptide_chain_ref, backbone_atoms,
            )
...
def main() -> None:
    args = parse_args()
    backbone_atoms, cutoffs, resolved_workers, peptide_chain_ref, peptide_chain_pred = validate_args(args)
    ...
    result = collect_metrics(
        ...
        peptide_chain_pred=peptide_chain_pred,
        peptide_chain_ref=peptide_chain_ref,
